...
- FE：统一 GET 参数传递规范，修复 `params` 不一致风险
- BE：补齐搜索接口参数校验与分页上限
...
- 三类搜索接口参数行为一致
- 不再出现“看似成功但实际未带条件”的请求

### 8.2 P1（5~7 天，可上线）

- FE：
  - 新搜索页交互（综合/消息/联系人/群聊/文件与媒体）
  - 统一结果模型 + 混排
  - 搜索历史与热词保留
- BE：
  - 联系人搜索改 DB 条件检索
...
- BE：新增 `/system/im/search/global` 聚合接口 + 统一打分排序
- FE：改为单接口为主，分源接口为降级兜底
