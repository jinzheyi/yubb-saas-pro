...
class chat {
  ...
  onMessage(data) {
    ...
    switch (res.msg) {
      case "fail":
        if (
          res.data == "Token已过期!请重新登录" || res.data == "你已被禁用" ||
          res.data == "用户不存在"
        ) {
          // 退出登录
  ...
  recall(message) {
    return new Promise((result, reject) => {
      $H.post("/chat/recall", {
        ...
        // key值：chatDetail_当前用户id_会话类型_接收人/群id
        let key =
          `chatDetail_${this.user.id}_${message.chat_type}_${message.to_id}`;
        // 获取原来的聊天记录
        let list = this.getChatDetail(key);
        // 根据k查找对应聊天记录
        let index = list.findIndex((item) => item.id === message.id);
  ...
  send(message, onProgress = false) {
    return new Promise(async (result, reject) => {
      ...
      // 上传文件
      let isUpload = message.type !== "text" && message.type !== "emoticon" &&
        message.type !== "card" &&
        !message.data.startsWith("http://tangzhe123-com");
  ...
  addChatDetail(message, isSend = true) {
    console.log("添加聊天记录");
    // 获取对方id
    let id = message.chat_type === "user"
      ? (isSend ? message.to_id : message.from_id)
      : message.to_id;
    // key值：chatDetail_当前用户id_会话类型_接收人/群id
    let key = `chatDetail_${this.user.id}_${message.chat_type}_${id}`;
    // 获取原来的聊天记录
  ...
  async deleteChatDetailItem(message, isSend = true) {
    ...
    let index = list.findIndex((item) =>
      item.k === message.k || item.id === message.id
    );
    if (index === -1) return;
    list.splice(index, 1);
    // 存储
  ...
  async updateChatItem(where, data) {
    ...
    // 找到当前会话
    let index = list.findIndex((item) =>
      item.id === where.id && item.chat_type === where.chat_type
    );
    if (index === -1) return;
    // 更新数据
  ...
  async readChatItem(id, chat_type) {
    ...
    // 找到当前会话
    let index = list.findIndex((item) =>
      item.id === id && item.chat_type === chat_type
    );
    if (index !== -1) {
