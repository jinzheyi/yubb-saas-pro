...
class _EditableTextState extends State<_EditableText>
  ...
  void _handleSelectionChanged(
    ...
    if (selection.start < 0 || selection.start > widget.controller.text.length || selection.end < 0 || selection.end > widget.controller.text.length) {
      return;
    }
  ...
  void _updateComposingRectIfNeeded() {
    ...
    assert(mounted);
    // Composing range may be invalid when using special text (emoji), 
    // skip update if invalid to avoid assertion failure
    ...
    if (composingRect == null) {
      final int offset = composingRange.isCollapsed ? composingRange.start : 0;
      composingRect =
  ...
  late final Map<Type, Action<Intent>> _actions = <Type, Action<Intent>>{
    ...
    // ExtendSelectionByPageIntent is removed in Flutter 3.44
