...
圣钰 SaaS Pro 面向组织内部的即时消息、协同办公、文件交换及互联网实时协作。产品不提供运营商网络接入、号码资源经营或公共电话服务能力；语音/视频功能仅用于经授权用户之间的互联网协作。

项目采用 Apache License 2.0。许可证允许在满足其条件的前提下使用、修改和再分发源码，但不授予“圣钰”“钰信”等名称、标识、商标、域名和品牌素材的使用许可；第三方依赖及服务适用各自的许可证和服务条款。

部署者、集成方及实际运营者应对自身的产品、用户、数据、内容、网络安全和对外服务独立负责，并依法落实适用的主体资质、备案、隐私保护、账号治理、服务规则和风险处置要求。不得将本项目用于任何违反适用法律、损害他人合法权益或破坏网络安全的用途。

项目方计划在未来以独立项目和独立协议建设面向企业协作场景的通讯信令 SDK 开放体系；该规划不代表当前提供通信资源、运营许可或任何服务承诺。涉及对外运营、跨境数据、AI 能力或新增行业功能时，请在上线前完成专项合规评估。

完整的适用范围、运营者义务、投诉举报与责任边界，请阅读官网的[《合规使用与责任边界说明》](https://shengyukj.top/guide/explain/compliance-notice)。该说明不构成法律意见，也不排除任何主体依法应承担的责任。
