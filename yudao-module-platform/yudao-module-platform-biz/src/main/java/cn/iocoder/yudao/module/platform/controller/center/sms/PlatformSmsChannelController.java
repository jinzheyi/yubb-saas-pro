...
@Api(tags = "管理后台 - 短信渠道")
@RestController
@RequestMapping("/platform/sms-channel")
public class PlatformSmsChannelController {
    ...
    @PostMapping("/create")
    @ApiOperation("创建短信渠道")
    @PreAuthorize("@ss.hasPermission('platform:sms-channel:create')")
    ...
    @PutMapping("/update")
    @ApiOperation("更新短信渠道")
    @PreAuthorize("@ss.hasPermission('platform:sms-channel:update')")
    ...
    @PreAuthorize("@ss.hasPermission('platform:sms-channel:delete')")
    ...
    @PreAuthorize("@ss.hasPermission('platform:sms-channel:query')")
    public CommonResult<SmsChannelRespVO> getSmsChannel(@RequestParam("id") Long id) {
        PlatformSmsChannelDO smsChannel = platformSmsChannelService.getSmsChannel(id);
    ...
    public CommonResult<PageResult<SmsChannelRespVO>> getSmsChannelPage(@Valid SmsChannelPageReqVO pageVO) {
        PageResult<PlatformSmsChannelDO> pageResult = platformSmsChannelService.getSmsChannelPage(pageVO);
    ...
    public CommonResult<List<SmsChannelSimpleRespVO>> getSimpleSmsChannels() {
        List<PlatformSmsChannelDO> list = platformSmsChannelService.getSmsChannelList();
        // 排序后，返回给前端
        list.sort(Comparator.comparing(PlatformSmsChannelDO::getId));
