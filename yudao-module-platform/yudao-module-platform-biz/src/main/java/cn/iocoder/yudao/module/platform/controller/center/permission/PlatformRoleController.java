...
public class PlatformRoleController {
    ...
    @PostMapping("/create")
    @ApiOperation("创建角色")
    @PreAuthorize("@ss.hasPermission('platform:role:create')")
    ...
    @PutMapping("/update")
    @ApiOperation("修改角色")
    @PreAuthorize("@ss.hasPermission('platform:role:update')")
    ...
    @PreAuthorize("@ss.hasPermission('platform:role:delete')")
    ...
    @GetMapping("/get")
    @ApiOperation("获得角色信息")
    @PreAuthorize("@ss.hasPermission('platform:role:query')")
    public CommonResult<RoleRespVO> getRole(@RequestParam("id") Long id) {
        PlatformRoleDO role = roleService.getRole(id);
    ...
    public CommonResult<List<RoleSimpleRespVO>> getSimpleRoles() {
        // 获得角色列表，只要开启状态的
        List<PlatformRoleDO> list = roleService.getRoles(Collections.singleton(CommonStatusEnum.ENABLE.getStatus()));
        // 排序后，返回给前端
        list.sort(Comparator.comparing(PlatformRoleDO::getSort));
        return success(RoleConvert.INSTANCE.convertList02(list));
    }

    // TODO 这里的@OperateLog的日志注解，后期要考虑下平台日志的独立性，大概率另起一个平台日志注解
    @GetMapping("/export")
    //@OperateLog(type = EXPORT)
    @PreAuthorize("@ss.hasPermission('platform:role:export')")
    public void export(HttpServletResponse response, @Validated RoleExportReqVO reqVO) throws IOException {
        List<PlatformRoleDO> list = roleService.getRoleList(reqVO);
