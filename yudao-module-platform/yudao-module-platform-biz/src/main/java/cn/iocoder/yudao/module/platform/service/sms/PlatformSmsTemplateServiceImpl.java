...
import cn.iocoder.yudao.module.platform.dal.mapper.sms.PlatformSmsTemplateMapper;
...
@Service
@Slf4j
public class PlatformSmsTemplateServiceImpl implements PlatformSmsTemplateService {
    ...
    @Resource
    private PlatformSmsProducer platformSmsProducer;

    /**
     * 短信模板缓存
     * key：短信模板编码 {@link PlatformSmsTemplateDO#getCode()}
     *
     * 这里声明 volatile 修饰的原因是，每次刷新时，直接修改指向
     */
    private volatile Map<String, PlatformSmsTemplateDO> smsTemplateCache;
    /**
    ...
    public void initLocalCache() {
        // 获取短信模板列表，如果有更新
        List<PlatformSmsTemplateDO> smsTemplateList = this.loadSmsTemplateIfUpdate(maxUpdateTime);
        if (CollUtil.isEmpty(smsTemplateList)) {
            return;
        }

        // 写入缓存
        smsTemplateCache = CollectionUtils.convertMap(smsTemplateList, PlatformSmsTemplateDO::getCode);
        maxUpdateTime = CollectionUtils.getMaxValue(smsTemplateList, PlatformSmsTemplateDO::getUpdateTime);
     ...
     * @return 短信模板列表
     */
    private List<PlatformSmsTemplateDO> loadSmsTemplateIfUpdate(Date maxUpdateTime) {
        // 第一步，判断是否要更新。
        if (maxUpdateTime == null) { // 如果更新时间为空，说明 DB 一定有新数据
            log.info("[loadSmsTemplateIfUpdate][首次加载全量短信模板]");
        } else { // 判断数据库中是否有更新的短信模板
            if (platformSmsTemplateMapper.selectCountByUpdateTimeGt(maxUpdateTime) == 0) {
                return null;
            }
            log.info("[loadSmsTemplateIfUpdate][增量加载全量短信模板]");
        }
        // 第二步，如果有更新，则从数据库加载所有短信模板
        return platformSmsTemplateMapper.selectList();
    }
    ...
    @Override
    public PlatformSmsTemplateDO getSmsTemplateByCodeFromCache(String code) {
    ...
    @Override
    public PlatformSmsTemplateDO getSmsTemplateByCode(String code) {
        return platformSmsTemplateMapper.selectByCode(code);
    }
    ...
    public Long createSmsTemplate(SmsTemplateCreateReqVO createReqVO) {
        // 校验短信渠道
        PlatformSmsChannelDO channelDO = checkSmsChannel(createReqVO.getChannelId());
        // 校验短信编码是否重复
        ...
        // 插入
        PlatformSmsTemplateDO template = SmsTemplateConvert.INSTANCE.convert(createReqVO);
        template.setParams(parseTemplateContentParams(template.getContent()));
        template.setChannelCode(channelDO.getCode());
        platformSmsTemplateMapper.insert(template);
        // 发送刷新消息
        platformSmsProducer.sendSmsTemplateRefreshMessage();
        // 返回
    ...
    public void updateSmsTemplate(SmsTemplateUpdateReqVO updateReqVO) {
        ...
        // 校验短信渠道
        PlatformSmsChannelDO channelDO = checkSmsChannel(updateReqVO.getChannelId());
        // 校验短信编码是否重复
        ...
        // 更新
        PlatformSmsTemplateDO updateObj = SmsTemplateConvert.INSTANCE.convert(updateReqVO);
        updateObj.setParams(parseTemplateContentParams(updateObj.getContent()));
        updateObj.setChannelCode(channelDO.getCode());
        platformSmsTemplateMapper.updateById(updateObj);
        // 发送刷新消息
        platformSmsProducer.sendSmsTemplateRefreshMessage();
    }
    ...
    public void deleteSmsTemplate(Long id) {
        ...
        // 更新
        platformSmsTemplateMapper.deleteById(id);
        // 发送刷新消息
        platformSmsProducer.sendSmsTemplateRefreshMessage();
    }

    private void validateSmsTemplateExists(Long id) {
        if (platformSmsTemplateMapper.selectById(id) == null) {
            throw exception(SMS_TEMPLATE_NOT_EXISTS);
        }
    }

    @Override
    public PlatformSmsTemplateDO getSmsTemplate(Long id) {
        return platformSmsTemplateMapper.selectById(id);
    }

    @Override
    public List<PlatformSmsTemplateDO> getSmsTemplateList(Collection<Long> ids) {
        return platformSmsTemplateMapper.selectBatchIds(ids);
    }

    @Override
    public PageResult<PlatformSmsTemplateDO> getSmsTemplatePage(SmsTemplatePageReqVO pageReqVO) {
        return platformSmsTemplateMapper.selectPage(pageReqVO);
    }

    @Override
    public List<PlatformSmsTemplateDO> getSmsTemplateList(SmsTemplateExportReqVO exportReqVO) {
        return platformSmsTemplateMapper.selectList(exportReqVO);
    }

    @Override
    public Long countByChannelId(Long channelId) {
        return platformSmsTemplateMapper.selectCountByChannelId(channelId);
    }

    @VisibleForTesting
    public PlatformSmsChannelDO checkSmsChannel(Long channelId) {
        PlatformSmsChannelDO channelDO = platformSmsChannelService.getSmsChannel(channelId);
        if (channelDO == null) {
    ...
    public void checkSmsTemplateCodeDuplicate(Long id, String code) {
        PlatformSmsTemplateDO template = platformSmsTemplateMapper.selectByCode(code);
        if (template == null) {
            return;
        }
