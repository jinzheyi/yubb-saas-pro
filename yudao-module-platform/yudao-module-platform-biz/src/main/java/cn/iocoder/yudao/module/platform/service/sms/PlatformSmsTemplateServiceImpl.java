...
import cn.iocoder.yudao.module.platform.dal.mysql.sms.PlatformSmsTemplateMapper;
...
public class PlatformSmsTemplateServiceImpl implements PlatformSmsTemplateService {
    ...
    public SmsChannelDO checkSmsChannel(Long channelId) {
        SmsChannelDO channelDO = platformSmsChannelService.getSmsChannel(channelId);
        if (channelDO == null) {
    ...
    public void checkSmsTemplateCodeDuplicate(Long id, String code) {
        SmsTemplateDO template = platformSmsTemplateMapper.selectByCode(code);
        if (template == null) {
            return;
        }
