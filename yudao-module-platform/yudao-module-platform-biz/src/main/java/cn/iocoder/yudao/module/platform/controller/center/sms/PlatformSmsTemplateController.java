...
@Api(tags = "管理后台 - 短信模板")
@RestController
@RequestMapping("/platform/sms-template")
public class PlatformSmsTemplateController {
    ...
    @PostMapping("/create")
    @ApiOperation("创建短信模板")
    @PreAuthorize("@ss.hasPermission('platform:sms-template:create')")
    ...
    @PutMapping("/update")
    @ApiOperation("更新短信模板")
    @PreAuthorize("@ss.hasPermission('platform:sms-template:update')")
    ...
    @PreAuthorize("@ss.hasPermission('platform:sms-template:delete')")
    ...
    @PreAuthorize("@ss.hasPermission('platform:sms-template:query')")
    public CommonResult<SmsTemplateRespVO> getSmsTemplate(@RequestParam("id") Long id) {
        PlatformSmsTemplateDO smsTemplate = platformSmsTemplateService.getSmsTemplate(id);
    ...
    public CommonResult<PageResult<SmsTemplateRespVO>> getSmsTemplatePage(@Valid SmsTemplatePageReqVO pageVO) {
        PageResult<PlatformSmsTemplateDO> pageResult = platformSmsTemplateService.getSmsTemplatePage(pageVO);
        return success(SmsTemplateConvert.INSTANCE.convertPage(pageResult));
    }

    @GetMapping("/export-excel")
    @ApiOperation("导出短信模板 Excel")
    @PreAuthorize("@ss.hasPermission('platform:sms-template:export')")
    @OperateLog(type = EXPORT)
    public void exportSmsTemplateExcel(@Valid SmsTemplateExportReqVO exportReqVO,
                                       HttpServletResponse response) throws IOException {
        List<PlatformSmsTemplateDO> list = platformSmsTemplateService.getSmsTemplateList(exportReqVO);
        // 导出 Excel
    ...
    @ApiOperation("发送短信")
    @PreAuthorize("@ss.hasPermission('platform:sms-template:send-sms')")
