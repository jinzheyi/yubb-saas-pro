...
@Api(tags = "管理后台 - 短信日志")
@RestController
@RequestMapping("/platform/sms-log")
@Validated
public class PlatformSmsLogController {
    ...
    @PreAuthorize("@ss.hasPermission('platform:sms-log:query')")
    public CommonResult<PageResult<SmsLogRespVO>> getSmsLogPage(@Valid SmsLogPageReqVO pageVO) {
        PageResult<PlatformSmsLogDO> pageResult = platformSmsLogService.getSmsLogPage(pageVO);
        return success(SmsLogConvert.INSTANCE.convertPage(pageResult));
    }

    @GetMapping("/export-excel")
    @ApiOperation("导出短信日志 Excel")
    @PreAuthorize("@ss.hasPermission('platform:sms-log:export')")
    @OperateLog(type = EXPORT)
    public void exportSmsLogExcel(@Valid SmsLogExportReqVO exportReqVO,
                                  HttpServletResponse response) throws IOException {
        List<PlatformSmsLogDO> list = platformSmsLogService.getSmsLogList(exportReqVO);
        // 导出 Excel
