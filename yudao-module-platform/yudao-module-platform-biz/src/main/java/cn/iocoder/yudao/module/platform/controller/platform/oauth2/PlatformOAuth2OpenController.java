...
import cn.iocoder.yudao.module.platform.controller.platform.oauth2.vo.open.OAuth2OpenAccessTokenRespVO;
import cn.iocoder.yudao.module.platform.controller.platform.oauth2.vo.open.OAuth2OpenAuthorizeInfoRespVO;
import cn.iocoder.yudao.module.platform.controller.platform.oauth2.vo.open.OAuth2OpenCheckTokenRespVO;
...
import cn.iocoder.yudao.module.platform.service.oauth2.PlatformOAuth2GrantService;
 ...
 * scope 的使用示例，可见 {@link PlatformOAuth2UserController} 类
 *
 * @author 圣钰科技
 */
...
@Validated
@Slf4j
public class PlatformOAuth2OpenController {

    @Resource
    private PlatformOAuth2GrantService oauth2GrantServicePlatform;
    @Resource
    private PlatformOAuth2ClientService oauth2ClientServicePlatform;
    @Resource
    private PlatformOAuth2ApproveService oauth2ApproveServicePlatform;
    @Resource
    ...
    public CommonResult<OAuth2OpenAccessTokenRespVO> postAccessToken(HttpServletRequest request,
        ...
        switch (grantTypeEnum) {
            case AUTHORIZATION_CODE:
                ...
                break;
            case PASSWORD:
                accessTokenDO = oauth2GrantServicePlatform.grantPassword(username, password, client.getClientId(), scopes);
                break;
            case CLIENT_CREDENTIALS:
                accessTokenDO = oauth2GrantServicePlatform.grantClientCredentials(client.getClientId(), scopes);
                break;
            case REFRESH_TOKEN:
                accessTokenDO = oauth2GrantServicePlatform.grantRefreshToken(refreshToken, client.getClientId());
                break;
            default:
    ...
    public CommonResult<Boolean> revokeToken(HttpServletRequest request,
        ...
        // 删除访问令牌
        return success(oauth2GrantServicePlatform.revokeToken(client.getClientId(), token));
    }
    ...
    public CommonResult<OAuth2OpenCheckTokenRespVO> checkToken(HttpServletRequest request,
        ...
        // 校验客户端
        String[] clientIdAndSecret = obtainBasicAuthorization(request);
        oauth2ClientServicePlatform.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1],
                null, null, null);
    ...
    public CommonResult<OAuth2OpenAuthorizeInfoRespVO> authorize(@RequestParam("clientId") String clientId) {
        ...
        // 2. 获得用户已经授权的信息
        List<PlatformOAuth2ApproveDO> approves = oauth2ApproveServicePlatform.getApproveList(getPlatformLoginUserId(), getUserType(), clientId);
        // 拼接返回
    ...
    public CommonResult<String> approveOrDeny(@RequestParam("response_type") String responseType,
        ...
        if (Boolean.TRUE.equals(autoApprove)) {
            ...
            if (!oauth2ApproveServicePlatform.checkForPreApproval(getPlatformLoginUserId(), getUserType(), clientId, scopes.keySet())) {
                return success(null);
            }
        } else { // 2.2 假设 approved 非 null，说明是场景二
            // 如果计算后不通过，则跳转一个错误链接
            if (!oauth2ApproveServicePlatform.updateAfterApproval(getPlatformLoginUserId(), getUserType(), clientId, scopes)) {
        ...
        if (grantTypeEnum == OAuth2GrantTypeEnum.AUTHORIZATION_CODE) {
            return success(getAuthorizationCodeRedirect(getPlatformLoginUserId(), client, approveScopes, redirectUri, state));
        }
        // 3.2 如果是 token 则是 implicit 简化模式，则发送 accessToken 访问令牌，并重定向
        return success(getImplicitGrantRedirect(getPlatformLoginUserId(), client, approveScopes, redirectUri, state));
    }
