 ...
 */
@Service
public class PlatformSmsSendServiceImpl implements PlatformSmsSendService {
    ...
    @Override
    public Long sendSingleSmsToUser(String mobile, Long userId, String templateCode, Map<String, Object> templateParams) {
        ...
        // 执行发送
        return this.sendSingleSms(mobile, userId, UserTypeEnum.PLATFORM_ADMIN.getValue(), templateCode, templateParams);
    }
    ...
    public Long sendSingleSmsToMember(String mobile, Long userId, String templateCode, Map<String, Object> templateParams) {
        // 如果 mobile 为空，则加载用户编号对应的手机号
        if (StrUtil.isEmpty(mobile)) {
            UserRespDTO member = memberUserApi.getUser(userId);
            if (member != null) {
    ...
    public Long sendSingleSms(String mobile, Long userId, Integer userType,
        ...
        // 校验短信模板是否合法
        PlatformSmsTemplateDO template = this.checkSmsTemplateValid(templateCode);
        // 校验手机号码是否存在
        ...
        String content = platformSmsTemplateService.formatSmsTemplateContent(template.getContent(), templateParams);
        Long sendLogId = platformSmsLogService.createSmsLog(mobile, userId, userType, isSend, template, content, templateParams);

        // 发送 MQ 消息，异步执行发送短信
        if (isSend) {
            platformSmsProducer.sendSmsSendMessage(sendLogId, mobile, template.getChannelId(),
    ...
    @VisibleForTesting
    public PlatformSmsTemplateDO checkSmsTemplateValid(String templateCode) {
        // 获得短信模板。考虑到效率，从缓存中获取
        PlatformSmsTemplateDO template = platformSmsTemplateService.getSmsTemplateByCodeFromCache(templateCode);
        // 短信模板不存在
        if (template == null) {
     ...
     */
    @VisibleForTesting
    public List<KeyValue<String, Object>> buildTemplateParams(PlatformSmsTemplateDO template, Map<String, Object> templateParams) {
    ...
    @Override
    public void doSendSms(PlatformSmsSendMessage message) {
        ...
        platformSmsLogService.updateSmsSendResult(message.getLogId(), sendResult.getCode(), sendResult.getMsg(),
    ...
    public void receiveSmsStatus(String channelCode, String text) throws Throwable {
        ...
        receiveResults.forEach(result -> platformSmsLogService.updateSmsReceiveResult(result.getLogId(),
