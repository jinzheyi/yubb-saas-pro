package cn.iocoder.yudao.module.infra.controller.center.config;
...
import cn.iocoder.yudao.module.infra.controller.center.config.vo.*;
...
public class ConfigController {
    ...
    @PostMapping("/create")
    @ApiOperation("创建参数配置")
    @PreAuthorize("@cs.hasPermission('infra:config:create')")
    ...
    @PutMapping("/update")
    @ApiOperation("修改参数配置")
    @PreAuthorize("@cs.hasPermission('infra:config:update')")
    ...
    @PreAuthorize("@cs.hasPermission('infra:config:delete')")
    ...
    @ApiOperation("获得参数配置")
    @ApiImplicitParam(name = "id", value = "编号", required = true, example = "1024", dataTypeClass = Long.class)
    @PreAuthorize("@cs.hasPermission('infra:config:query')")
    ...
    @GetMapping("/export")
    @ApiOperation("导出参数配置")
    @PreAuthorize("@cs.hasPermission('infra:config:export')")
