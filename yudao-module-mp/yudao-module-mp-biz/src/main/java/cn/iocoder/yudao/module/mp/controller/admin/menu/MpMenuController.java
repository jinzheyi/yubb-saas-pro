...
public class MpMenuController {
    ...
    @PreAuthorize("@ss.hasPermission('mp:menu:save') && @ss.hasAllPlugApp('"+ MP_CODE +"')")
    ...
    @PreAuthorize("@ss.hasPermission('mp:menu:delete') && @ss.hasAllPlugApp('"+ MP_CODE +"')")
    ...
    @Parameter(name = "accountId", description = "公众号账号的编号", required = true, example = "10")
    @PreAuthorize("@ss.hasPermission('mp:menu:query') && @ss.hasAllPlugApp('"+ MP_CODE +"')")
