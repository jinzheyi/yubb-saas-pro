package com.shengyu.module.system.controller.admin.oauth2;
...
import com.shengyu.module.platform.api.oauth2.PlatformOAuth2ClientApi;
...
import com.shengyu.module.system.convert.oauth2.OAuth2OpenConvert;
...
import com.shengyu.framework.common.enums.oauth2.OAuth2GrantTypeEnum;
import com.shengyu.module.system.service.oauth2.OAuth2ApproveService;
import com.shengyu.module.system.service.oauth2.OAuth2GrantService;
...
public class OAuth2OpenController {
    ...
    public CommonResult<OAuth2OpenAccessTokenRespVO> postAccessToken(HttpServletRequest request,
        ...
        OAuth2ClientRespDTO clientRespDTO = auth2ClientApi.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1],
        ...
        switch (grantTypeEnum) {
            case AUTHORIZATION_CODE:
                accessTokenDO = oauth2GrantService.grantAuthorizationCodeForAccessToken(clientRespDTO.getClientId(), code, redirectUri, state);
                break;
            case PASSWORD:
                accessTokenDO = oauth2GrantService.grantPassword(username, password, clientRespDTO.getClientId(), scopes);
                break;
            case CLIENT_CREDENTIALS:
                accessTokenDO = oauth2GrantService.grantClientCredentials(clientRespDTO.getClientId(), scopes);
                break;
            case REFRESH_TOKEN:
                accessTokenDO = oauth2GrantService.grantRefreshToken(refreshToken, clientRespDTO.getClientId());
                break;
            default:
    ...
    public CommonResult<Boolean> revokeToken(HttpServletRequest request,
        ...
        // 删除访问令牌
        return success(oauth2GrantService.revokeToken(clientRespDTO.getClientId(), token));
    }
    ...
    public CommonResult<OAuth2OpenCheckTokenRespVO> checkToken(HttpServletRequest request,
        ...
        // 校验客户端
        String[] clientIdAndSecret = obtainBasicAuthorization(request);
        auth2ClientApi.validOAuthClientFromCache(clientIdAndSecret[0], clientIdAndSecret[1],
                null, null, null);
    ...
    public CommonResult<OAuth2OpenAuthorizeInfoRespVO> authorize(@RequestParam("clientId") String clientId) {
        ...
        // 拼接返回
        return success(OAuth2OpenConvert.INSTANCE.convert(clientRespDTO, approves));
    }
    ...
    public CommonResult<String> approveOrDeny(@RequestParam("response_type") String responseType,
        ...
        OAuth2ClientRespDTO clientRespDTO = auth2ClientApi.validOAuthClientFromCache(clientId, null,
        ...
        if (grantTypeEnum == OAuth2GrantTypeEnum.AUTHORIZATION_CODE) {
            return success(getAuthorizationCodeRedirect(getLoginUserId(), clientRespDTO, approveScopes, redirectUri, state));
        }
        // 3.2 如果是 token 则是 implicit 简化模式，则发送 accessToken 访问令牌，并重定向
        return success(getImplicitGrantRedirect(getLoginUserId(), clientRespDTO, approveScopes, redirectUri, state));
    }
    ...
    private String getImplicitGrantRedirect(Long userId, OAuth2ClientRespDTO clientRespDTO,
                                            List<String> scopes, String redirectUri, String state) {
        // 1. 创建 access token 访问令牌
        OAuth2AccessTokenDO accessTokenDO = oauth2GrantService.grantImplicit(userId, getUserType(), clientRespDTO.getClientId(), scopes);
        Assert.notNull(accessTokenDO, "访问令牌不能为空"); // 防御性检查
        // 2. 拼接重定向的 URL
        ...
        return OAuth2Utils.buildImplicitRedirectUri(redirectUri, accessTokenDO.getAccessToken(), state, accessTokenDO.getExpiresTime(),
                scopes, JsonUtils.parseObject(clientRespDTO.getAdditionalInformation(), Map.class));
    }

    private String getAuthorizationCodeRedirect(Long userId, OAuth2ClientRespDTO clientRespDTO,
                                                List<String> scopes, String redirectUri, String state) {
        // 1. 创建 code 授权码
        String authorizationCode = oauth2GrantService.grantAuthorizationCodeForCode(userId, getUserType(), clientRespDTO.getClientId(), scopes,
                redirectUri, state);
        // 2. 拼接重定向的 URL
