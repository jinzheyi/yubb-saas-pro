...
public interface ImChatMessageMapper extends BaseMapperX<ImChatMessageDO> {
    ...
    @Select({"<script>",
            ...
            "  AND (cw.clear_sequence IS NULL OR m.sequence &gt; cw.clear_sequence)",
            "  AND m.send_time &lt;= IFNULL(cu.left_at, '9999-12-31 23:59:59')",
            "  AND (m.status IS NULL OR m.status != ${@com.shengyu.module.system.enums.im.ImMessageStatusEnum@RECALLED.getStatus()})",
            "  AND m.message_type != ${@com.shengyu.module.system.enums.im.ImMessageTypeEnum@SYSTEM.getType()}",
            "<if test='chatId != null'>",
            "  AND m.chat_id = #{chatId}",
            "</if>",
    ...
    // ============================================================
    // 简化版搜索方法（无 JOIN，用于 Service 层先做权限校验后的查询）
    // ============================================================
    ...
    @Select({"<script>",
            ...
            "  AND m.deleted = 0",
            "  AND m.chat_id = #{chatId}",
            "  AND m.sequence &gt; #{clearSequence}",
            "<if test='leftAt != null'>",
            "  AND m.send_time &lt;= #{leftAt}",
            "</if>",
            "  AND (m.status IS NULL OR m.status != ${@com.shengyu.module.system.enums.im.ImMessageStatusEnum@RECALLED.getStatus()})",
            "  AND m.message_type != ${@com.shengyu.module.system.enums.im.ImMessageTypeEnum@SYSTEM.getType()}",
            "<if test='keyword != null and keyword != \"\"'>",
            ...
            "</if>",
            "</script>"})
    Long countSearchPageByChat(@Param("tenantId") Long tenantId,
                               @Param("chatId") Long chatId,
    ...
    @Select({"<script>",
            ...
            "LIMIT #{limit} OFFSET #{offset}",
            "</script>"})
    List<ImChatMessageDO> selectSearchPageByChat(@Param("tenantId") Long tenantId,
                                                 @Param("chatId") Long chatId,
                                                 @Param("clearSequence") Long clearSequence,
                                                 @Param("leftAt") java.time.LocalDateTime leftAt,
                                                 @Param("keyword") String keyword,
