...
public class AppDeptController {
    ...
    public CommonResult<List<AppDeptTreeRespVO>> getMyDeptTree() {
        ...
        Map<Long, Integer> deptSelectableMemberCountMap = new HashMap<>();
        for (Long deptId : allDeptsMap.keySet()) {
            ...
            int memberCount = CollUtil.isEmpty(deptUsers) ? 0 : deptUsers.size();
            deptMemberCountMap.put(deptId, memberCount);
            deptSelectableMemberCountMap.put(deptId, countSelectableMembers(deptUsers, userId));
        }
        ...
        for (DeptDO dept : allDeptsMap.values()) {
            ...
            vo.setSelectableMemberCount(deptSelectableMemberCountMap.getOrDefault(dept.getId(), 0));
            result.add(vo);
        }
    ...
    public CommonResult<List<AppDeptTreeRespVO>> getOrgTree() {
        ...
        for (DeptDO dept : allDepts) {
            ...
            int memberCount = CollUtil.isEmpty(deptUsers) ? 0 : deptUsers.size();
            deptMemberCountMap.put(dept.getId(), memberCount);
            deptSelectableMemberCountMap.put(dept.getId(), countSelectableMembers(deptUsers, userId));
        }
    ...
    private int countSelectableMembers(List<UserDeptDO> deptUsers, Long currentUserId) {
        if (CollUtil.isEmpty(deptUsers)) {
            return 0;
        }
        return (int) deptUsers.stream()
                .map(UserDeptDO::getUserId)
