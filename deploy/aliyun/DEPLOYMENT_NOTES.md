...
### 2026-09-10 Flutter Web 语音通话媒体前置检查
...
- 修复：所有端在创建或接听服务端通话前统一申请麦克风权限，视频额外申请摄像头。Web 通过浏览器 `getUserMedia` 进行短暂预检；浏览器没有设备或用户未授予权限时，不再创建邀请、加入 LiveKit 后立即取消，而是提示所需权限。
- 多端策略：Android/iOS 保持原生权限申请；Web 保持 HTTPS + 浏览器媒体设备要求。语音的听筒/扬声器初始路由改为连接成功后的非致命优化，特定 Android 音频栈路由失败时保留系统默认路由，不会终止已建立的媒体连接。
- 发布：仅已发布 Flutter Web 静态资源，容器旧资源备份为 `/usr/share/nginx/html.bak.20260910-063449`；未构建、未上传、未发布 Android/iOS/鸿蒙安装包及在线更新记录，等待 Web 多轮验证结束后统一发版。
