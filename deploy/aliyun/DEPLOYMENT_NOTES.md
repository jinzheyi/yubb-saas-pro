...
### 2026-09-10 Flutter 多端通话媒体授权与路由修复

- 现象与定位：生产 LiveKit 日志显示浏览器和 Android 均可完成信令、ICE/UDP 建连并进入 `participant active`，随后客户端以 `CLIENT_REQUEST_LEAVE` 主动离开；服务端、TLS、Nginx、令牌和安全组不是本次“通话连接失败”的根因。
- 授权策略：Android/iOS 在创建或接听前申请原生麦克风权限，视频额外申请摄像头；Web 不使用 `permission_handler` 预检，必须由 LiveKit 在启用本地媒体时触发浏览器原生 `getUserMedia` 授权弹窗。这样浏览器会正常询问用户是否允许麦克风/摄像头，而不是在页面初始化阶段直接视为拒绝。
- 多端路由策略：语音的听筒/扬声器初始路由改为连接成功后的非致命优化，特定 Android 音频栈路由失败时保留系统默认路由，不会终止已建立的媒体连接。Web 不执行原生音频路由调用。
- 发布：仅已发布 Flutter Web 静态资源，容器旧资源备份为 `/usr/share/nginx/html.bak.20260910-064954`；未构建、未上传、未发布 Android/iOS/鸿蒙安装包及在线更新记录，等待 Web 多轮验证结束后统一发版。

### 2026-09-10 Flutter App 全面会话边界审计

- 范围：覆盖登录、退出、账号切换、企业切换、令牌刷新、401 自动重试、会话列表、通讯录、群组、收藏、设备、更新检查、消息发送队列及 WebSocket 实时事件。
- 修复：登录态变更时同步释放旧账号的离线待发消息、会话预览内存缓存、聊天消息批处理、在线状态定时器和会话同步节流记录；并失效所有在创建时读取用户编号的 Provider/数据源，避免旧账号请求或内存状态进入新账号页面。
- 异步保护：令牌刷新仅在发起刷新时的账号、租户及双 Token 仍为当前会话时写回；旧请求的 400/401 刷新失败只会清理持有同一 Authorization 的当前会话，不会将刚完成登录或切换企业的用户误登出。
- 消息队列：网络恢复监听随 Provider 释放取消；账号切换时待发队列及其回调被明确释放，避免旧账号消息使用新账号 Token 重试。
- 数据策略：本地持久化数据继续按用户编号读取，不执行全表删除，兼顾多账号隔离与再次登录性能。原生 App 需随下一次正式版本安装包或在线更新发布后获得这些修复。
...
### 2026-09-10 Flutter 账号切换企业列表隔离修复

- 现象：App 使用账号 B 登录后，切换企业页面仍展示账号 A 的内存企业列表。根因是该全局 Riverpod 状态未随登录用户切换清理，页面仅在列表为空时才重新请求。
- 修复：登录用户变化时主动失效企业列表 Provider；列表状态记录所属 SaaS 用户，页面仅复用当前用户数据；企业列表接口异步返回时再次核验当前用户，丢弃已切换账号的旧请求结果。
- 发布：已构建并替换生产 `shengyu-im-flutter-web` 静态资源，`http://127.0.0.1:8082/` 返回 `200`；旧资源保留在容器 `/usr/share/nginx/html.bak.20260910-054933`。原生 App 需包含本次代码的新安装包或在线更新版本后生效。
