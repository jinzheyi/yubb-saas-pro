...
import * as MenuApi from '@/api/system/tenantMenu'
import { CACHE_KEY, useCache } from '@/hooks/web/useCache'
import {CommonStatusEnum, MenuDimensionEnum, SystemMenuTypeEnum} from '@/utils/constants'
import { defaultProps, handleTree } from '@/utils/tree'
import {getSimpleTenantPlugMenusList} from "@/api/system/tenantMenu";

defineOptions({ name: 'PlugAppMenuForm' })
...
const formData = ref({
  ...
  keepAlive: true,
  alwaysShow: true,
  dimension: MenuDimensionEnum.PLUG,
  plugAppSn: ''
})
...
/** 打开弹窗 */
const open = async (type: string, id?: number, parentId?: number, plugAppSn: string) => {
  dialogVisible.value = true
  dialogTitle.value = t('action.' + type)
  formType.value = type
  formData.value.plugAppSn = plugAppSn
  plugAppSnValue.value = plugAppSn
  resetForm()
  if (parentId) {
  ...
  if (id) {
    ...
    try {
      formData.value = await MenuApi.getTenantMenu(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    formData.value.plugAppSn = plugAppSnValue.value
    const data = formData.value as unknown as MenuApi.TenantMenuVO
    if (formType.value === 'create') {
      await MenuApi.createTenantMenu(data)
      message.success(t('common.createSuccess'))
    } else {
      await MenuApi.updateTenantMenu(data)
...
const getTree = async () => {
  menuTree.value = []
  const res = await MenuApi.getSimpleTenantPlugMenusList(plugAppSnValue.value)
